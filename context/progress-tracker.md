...
- **Edge Label Pills**: Added inline text labels to canvas edges. Double-clicking any edge path opens a pill-shaped inline input at the midpoint. The committed label renders as a styled dark pill. Label is persisted in `CanvasEdgeData.label` and survives autosave/reload.
